...
alias Point = {x: Int32, y: Int32, z: Int32}
...
def generate_neighbours(point : Point) : Array(Point)
  ...
  (-1..1).each do |x|
    (-1..1).each do |y|
      (-1..1).each do |z|
        pos = {x: point[:x] + x, y: point[:y] + y, z: point[:z] + z}
        neighbours << pos unless x == 0 && y == 0 && z == 0
      end
    end
  end
  neighbours
...
def solve_a(cyclesNum : Int32, input : Hash(Point, Cube)) : Int32
